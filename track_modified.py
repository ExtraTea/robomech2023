...
def run(
    ...
    node = nep.node('cropped_camera_publisher')
    conf = node.hybrid("127.0.0.1")
    pub_cropped_image = node.new_pub('cropped_image','image', conf)
    print ("waiting service")
    sys.stdout.flush()
    print ("service started")
    sys.stdout.flush()
    mode = 0 #0:search 1:loading 2:defined
    area_size = 0
    maxi_area_size = 0
    arg_maxi_area = None
    source = str(source)
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        for i, det in enumerate(p):  # detections per image
            ...
            if webcam:  # bs >= 1
                ...
                original_pic = copy.deepcopy(im0)
                print(original_pic.shape)
            else:
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        if mode == 0:
                            area_size = (output[2] - output[0]) * (output[3] - output[1])
                            if maxi_area_size < area_size:
                                arg_maxi_area = output[4]
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        ###trimmer
                        #aaa = annotator.result()
                        #cv2.imshow(str(0),original_pic)
                        #cv2.waitKey(1)
                        ...
                        if mode == 0:
                            key = cv2.waitKey(10)
                            if key == -1:
                                print("Plz push any button to define target.")
                            else:
                                print("The target is", arg_maxi_area)
                                print("if wrong, please push any button in 5 seconds")
                                mode = 1
                        elif mode == 1:
                            ...
                            if key == -1:
                                ...
                                mode = 2
                                target = arg_maxi_area
                            else:
                                print("reset target. When ready, plz push any button.")
                                mode = 0
                        
                        if(mode == 2 and id == arg_maxi_area):

                            b_left = max(int(output[0]), 0)
                            b_top = max(int(output[1]), 0)
                            b_right = min(int(output[2]), 1920)
                            b_bottom = min(int(output[3]), 1080)
                            #if(b_top!=b_bottom or b_left!=b_right):
                            cropped_pic = original_pic[b_top:b_bottom, b_left:b_right]
                                #cv2.imshow(str(1),cropped_pic)
                            print(cropped_pic.shape)
                            pub_cropped_image.publish(cropped_pic)
            ...
            if show_vid:
                ...
                cv2.imshow(str(p), im0) #original
                #cv2.imshow(str("a"), im0) #handmade
