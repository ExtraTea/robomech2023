...
def thread_function(name):
    global img00, sub_image, pub_image
    while True:
...
node = nep.node('camera_web_listener')
conf = node.hybrid("127.0.0.1") # change the IP displayed when using nep master wifi
sub_image = node.new_sub('test_image','image',conf)
...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.frames[i] = 0 or float('inf')  # infinite stream fallback
            self.fps[i] = 0 or 30  # 30 FPS fallback

            self.imgs[i] = cv2.imread(r"C:\Users\daina\OneDrive\Pictures\wallpaper\resized.jpg")  # guarantee first frame
